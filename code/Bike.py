...
def MkDataSource():
    ...
    def PrepareTrainData(): 
        ...
        nonlocal trainLine
        nonlocal validLine
        lines = DataPreprocess(Path+"data/hour.csv")
        print(lines.map(lambda fields: fields[0].split(',')[16]).take(5))
        categoriesMap = lines.map(lambda fields: fields[0].split(',')).distinct().zipWithIndex().collectAsMap()
        print("catagoriesMap = ")
        #print(categoriesMap)
